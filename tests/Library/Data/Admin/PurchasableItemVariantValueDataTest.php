...
class PurchasableItemVariantValueDataTest extends KernelTestCase
{
    ...
    public function testPurchasableItemVariantValue(): void
    {
        $this->assertSame($this->purchasableItemVariantValue, $this->data->getPurchasableItemVariantValue());
    }
    ...
    public function testUniqueValidation(): void
    {
        ...
        $value = $this->purchasableItemVariantValueRepository->findOneById($uid);

        $data = new PurchasableItemVariantValueData($value, $value->getPurchasableItemVariant());
        $data->setPriority(100);

        // editing (no change of name)
        $data->setName('Value 1');
        $result = $this->validator->validate($data);
        $this->assertEmpty($result); // valid

        // editing (new name)
        $data->setName('Value 3');
        $result = $this->validator->validate($data);
        $this->assertEmpty($result); // valid

        // editing (existing name)
        $data->setName('Value 2');
        $result = $this->validator->validate($data);
        $this->assertNotEmpty($result); // invalid

        $data = new PurchasableItemVariantValueData(null, $value->getPurchasableItemVariant());
        $data->setPriority(100);
    ...
    protected function setUp(): void
    {
        ...
        /** @var PurchasableItemVariantValueRepositoryInterface $repository */
        $repository = $container->get(PurchasableItemVariantValueRepositoryInterface::class);
        $this->purchasableItemVariantValueRepository = $repository;
        ...
        $this->data = new PurchasableItemVariantValueData($this->purchasableItemVariantValue, $this->purchasableItemVariant);
    }
}